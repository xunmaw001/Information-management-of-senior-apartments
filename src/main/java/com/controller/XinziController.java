    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(xinzi !=null){
            ...
            BeanUtils.copyProperties( xinzi , view );//把实体数据重构到view中
                ...
                //级联表
                HugongEntity hugong = hugongService.selectById(xinzi.getHugongId());
                if(hugong != null){
    ...
    */
    ...
    public R save(@RequestBody XinziEntity xinzi, HttpServletRequest request){
        ...
        if(false)
            ...
            xinzi.setHugongId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<XinziEntity> queryWrapper = new EntityWrapper<XinziEntity>()
            .eq("hugong_id", xinzi.getHugongId())
            .eq("xinzi_types", xinzi.getXinziTypes())
            ;
...
//                            xinziEntity.setHugongId(Integer.valueOf(data.get(0)));   //护工 要改的
//                            xinziEntity.setXinziTypes(Integer.valueOf(data.get(0)));   //类型 要改的
//                            xinziEntity.setXinziNumber(data.get(0));                    //薪资数量 要改的
